...
class RtpDataSource : BaseDataSource(true) {
    ...
    override fun open(dataSpec: DataSpec): Long {
        ...
        socket = MulticastSocket(null).apply {
            reuseAddress = true
            soTimeout = SP.videoPlayerLoadTimeout.toInt()
    ...
    override fun read(buffer: ByteArray, offset: Int, length: Int): Int {
        ...
        while (payloadOffset >= payloadLength) readNextPayload()
    ...
    override fun close() {
        ...
        if (currentSocket != null && group?.isMulticastAddress == true) {
            runCatching { currentSocket.leaveGroup(group) }
        }
        currentSocket?.close()
        socket = null
        groupAddress = null
    ...
    private fun readNextPayload() {
        ...
        (socket ?: throw IOException("RTP socket is not open")).receive(packet)
        val start = rtpPayloadOffset(datagramBuffer, packet.offset, packet.length)
        val end = rtpPayloadEnd(datagramBuffer, packet.offset, packet.length)
        if (start >= end) {
            payloadOffset = 0
            payloadLength = 0
            return
        }
        payloadLength = end - start
        System.arraycopy(datagramBuffer, start, payloadBuffer, 0, payloadLength)
        payloadOffset = 0
    }
    ...
    private fun rtpPayloadOffset(data: ByteArray, offset: Int, length: Int): Int {
        ...
        if ((data[offset].toInt() and RTP_EXTENSION_MASK) != 0) {
            if (headerLength + RTP_EXTENSION_HEADER_SIZE > length) return offset
            val words = ((data[offset + headerLength + 2].toInt() and 0xFF) shl 8) or
                (data[offset + headerLength + 3].toInt() and 0xFF)
            headerLength += RTP_EXTENSION_HEADER_SIZE + words * RTP_EXTENSION_WORD_SIZE
        }
    ...
    private fun rtpPayloadEnd(data: ByteArray, offset: Int, length: Int): Int {
        ...
        if (length >= RTP_FIXED_HEADER_SIZE && (data[offset].toInt() and RTP_PADDING_MASK) != 0) {
